...
for k = 1:numOfTrainSamples    %Iterating all the training data
    for e = 1:epochs           %Maximum Epochs
        ...
        %Total Error
        totalOutputError=0.5*(sum(outputLayerPerceptrons-vecTrainOutputs(:,k)));
     
        %Backward Pass
        %Calculating DeltaOutput and DeltaHidden 
        %Sk= Ok(1-Ok)(Ok-tk)
        deltaOutput=outputLayerPerceptrons.*(ones(size(outputLayerPerceptrons))-outputLayerPerceptrons).*(outputLayerPerceptrons-vecTrainOutputs(:,k));
...
for k=1:numOfTestSamples  %for the whole test set
    ...
    outputLayerPerceptronsTest=softmax((sumTempHiddenTest)); %+tempBiasHidden
    
    classifiedData(:,k)=outputLayerPerceptronsTest;
